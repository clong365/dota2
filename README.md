...
每局一行(队伍A/B = 系列赛对阵双方,取 Game 1 的天辉/夜魇;队伍名为 OpenDota 官方英文名,无中文字段):

| 列 | 说明 |
|---|---|
| 开赛时间 | UTC+8 |
| 队伍A / 队伍B | **绿色 = 该局胜者** |
| 赛制 / 赛果 | 按 series_id 归组,BO1/BO3/BO5;赛果 = 系列赛大比分 |
| 局 / 单局比分 | Game 序号;比分恒为 队伍A:队伍B(系列赛内换边的局已翻转) |
| 用时 | mm:ss |
| 先10杀 / 一血 | 由 kills_log 时间戳、firstblood_claimed 推出 |
| 时长盘口 / 大赔率 / 小赔率 | 来自 etopfun;绿色 = 命中侧 |
| 人头盘口 / 大赔率 / 小赔率 | 总人头(双方击杀之和)大小盘;绿色 = 命中侧 |

顶部统计:均注大(每局买大,按 etopfun 盘口线判定)的命中局数与占比,时长盘、人头盘各一行。

底部派图(纯 SVG):均注大回收(命中局大赔率之和)与均注小回收(命中局小赔率之和)归一化为占比的两扇区派图,时长盘、人头盘各一个;港盘口径,回收为净利润之和,回收 > 未中局数即盈利。
...
- `SINCE` — 只展示该日期(UTC+8,如 `2026-08-20`)及以后的比赛,与 `DAYS` 取较严者;留空 = 不限。只影响抓取/展示,KV 缓存的历史详情保留
